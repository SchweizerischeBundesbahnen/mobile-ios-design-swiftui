...
BubbleView is a view that can only be used underneath the SBBNavigationBar. You need to set its image and title. Optional parameters are its subtitle and detail. It is by default in an expanded state (if detail is set), but you can overwrite the state if needed.
